...
const { roundTo10Minutes, getMidnightToday, isValidDateFormat, minutesFromMidnight } = require('./../utils/time')
...
module.exports = function weatherData(app) {
  ...
  async function _handleWeatherObservations(req, res) {
    ...
    const promises = {};
    promises.observation = weatherObservation(time)
  ...
  async function weatherObservation(time) {
    const params = {
      param:
        `stationname as name,lat,lon,distance,region,fmisid,utctime as time,${meteorologicalParameters}`,
      startTime: time !== undefined ? roundTo10Minutes(new Date(time)) : undefined,
      endtime: time !== undefined ? roundTo10Minutes(new Date(time)) : 'now',
      format: 'json',
      timeformat: 'sql',
      keyword: 'synop_fi',
      precision: 'full',
  ...
  app.use('/observation', cors(), _handleWeatherObservations);
}